...
#input - shape 'None' states that, the value can be anything, i.e. we can feed on any number of images
#input image
...
#first convolutional layer with 32 output filters, filter size 5x5, strdie of 2, same padding, and RELU activation.
...
print("Shape of first convolutional layer: {}".format(out_layer2.get_shape().as_list()))
...
validation_accuracy = round((sess.run(accuracy, feed_dict = {x: data.validation.images, y_: data.validatiom.labels})) * 100, 2)
print("Accurcay in the validation dataset: {}%".format(validation_accuracy))
...
cmp = np.where(idx == False) #indice of error outputs
# plotting errors
...
conv_layer1_output = (sess.run(out_layer1, feed_dict = {x:test_image}))
...
for i, ax in enumerate(axes.flat):
    ...
    xlabel = "Folter: {}".format(i + 1)
    ax.set_xlabel(xlabel)
    ax.set_xticks([])
    ax.set_yticks([])
fig.suptitle('Output of the convoloutuional layer')
plt.show()

im = Image.open("C:\\Users\\Anmol-Sachdeva\\Desktop\\seven.png")
print(im)
...
test_image1 = np.reshape(im2, [1, 784]) #reshape it to out input holder shape

pred = (sess.run(y_predicted, feed_dict = {x: test_image1}))
